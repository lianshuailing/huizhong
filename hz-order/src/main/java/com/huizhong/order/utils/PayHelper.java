package com.huizhong.order.utils;
...
import com.huizhong.order.config.PayConfig;
...
public class PayHelper {
     ...
     *
     * @param orderId 订单ID
     * @param totalFee 总金额 单位分
     * @param body   订单描述(商品描述)
     * @return
     */
    public String createPayUrl(Long orderId, Long totalFee, String body) {
        ...
        try {
            ...
            throw new HzException(ExceptionEnum.WX_PAY_ORDER_FAIL);
        }
    }
    ...
    public void isSuccess(Map<String, String> result) {
        ...
        if (FAIL.equals(result.get("result_code"))) {
            log.error("【微信下单】下单失败, 错误码：{}， 错误描述：{}", result.get("err_code"), result.get("err_code_des"));
            throw new HzException(ExceptionEnum.WX_PAY_ORDER_FAIL);
        }
    }
    ...
    public void isValidSign(Map<String, String> data) {
        // 重新生成签名, 和传过来的签名进行比较
        try {
            String sign1 = WXPayUtil.generateSignature(data, payConfig.getKey(), WXPayConstants.SignType.MD5);

            String sign2 = WXPayUtil.generateSignature(data, payConfig.getKey(), WXPayConstants.SignType.HMACSHA256);

            String sign = data.get("sign");

            if (!StringUtils.equals(sign, sign1) && !StringUtils.equals(sign, sign2)) {
                // 签名有误
                throw new HzException(ExceptionEnum.INVALID_SIGN_ERROR);
            }
        } catch (Exception e) {
